...
void Bilateral::initGrid() {
	...
	Mat L(6, gridSize, CV_32SC(3), Scalar::all(0));
	grid = L;
	int tSize = imgSrcArr.size();
	int xSize = imgSrcArr[0].rows;
	int ySize = imgSrcArr[0].cols;
					...
					for (int g = 0; g < gridSize[4]; g++) {
						for (int b = 0; b < gridSize[5]; b++) {
							...
							if (grid.at<Vec< int, 3 > >(point)[0] > 0) {
								...
								if (grid.at<Vec< int, 3 > >(point)[2] == 0) {
									fromSource = -log(bgdGMM(color));
									toSink = -log(fgdGMM(color));
								}
								else if (grid.at<Vec< int, 3 > >(point)[2] == -1) {
									fromSource = 0;
									toSink = 9999;
								}
								else {
								...
								if (t > 0) {
									...
									if (grid.at<Vec< int, 3 > >(pointN)[0] > 0) {
										double w = 0.5 * grid.at<Vec< int, 3 > >(point)[0] * grid.at<Vec< int, 3 > >(pointN)[0];
										w = sqrt(w);
								...
								if (r > 0) {
									...
									if (grid.at<Vec< int, 3 > >(pointN)[0] > 0) {
										double w = 0.2 * grid.at<Vec< int, 3 > >(point)[0] * grid.at<Vec< int, 3 > >(pointN)[0];
										w = sqrt(w);
										graph.addEdges(vtxIdx, grid.at<Vec< int, 3 > >(pointN)[1], w, w);
									}
								}
